...
int main(int argc, char** argv) {
  ...
  while(true) {
    ...
    if (op == "list") {  // Use case 1: Send the songs
      message n;
      n << "list" << songs.size();// puedo convertirlo en una cadena
      for(const auto& p : songs)
        n << p.first;// como song es una hastable asi se pone la llave que es el nombre de la cancion
      s.send(n);
    } else if(op == "play") {
      // Use case 2: Send song file
      string songName;// size_t es la entrada en la hastable;//el nombre del archivo que se quiere reproducir
      m >> songName;
